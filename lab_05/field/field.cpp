...
double Field::draw(QImage &image, int x, int y, QColor &font, QColor &line, QColor &fill, bool needFill, delayOut *dout) {
    clock_t start, end;
    start = clock();
    int x0 = 0, y0 = 0, x1 = 1, y1 = 1;
    findBorders(x0, y0, x1, y1);
    MyPixelMap pixelMap(x0, y0, x1 - x0, y1 - y0);
    pixelMap.drawFigures(figures, needFill, dout);
