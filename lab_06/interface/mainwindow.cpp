...
void MainWindow::paintEvent(QPaintEvent *pEvent) {
    ...
    if (needDelay != -1) {
        int dout = needDelay;
        field.draw(img, canvasWidth, canvasHeight, xf, yf, fontColor, linesColor, fillColor, tnf, &dout);
        if (dout == -1) {
            needDelay = -1;
        }
        else {
        ...
        double tmpTime = field.draw(img, canvasWidth, canvasHeight, xf, yf, fontColor, linesColor, fillColor, tnf);
        if (needFill) {
...
void MainWindow::on_aboutButton_clicked(void) {
    ...
    "Условие: Реализовать заполнение произвольного многоугольника с произвольным количеством отверстий алгоритмом построчного заполнения с затравкой.\n"
...
int MainWindow::getPixel(int &x, int &y) {
    int rc = 0;
...
void MainWindow::mousePressEvent(QMouseEvent *event) {
    ...
    if (cx >= canvasX && cx <= canvasX + canvasWidth && 
        ...
        if (event->button() == Qt::LeftButton || event->button() == Qt::RightButton) {
            ...
            this->ui->lineEditX_2->setText(tmpX);
            this->ui->lineEditY_2->setText(tmpY);
            needFill = false;
        }
    }
    repaint();
